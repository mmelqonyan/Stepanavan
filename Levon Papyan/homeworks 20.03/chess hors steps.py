...
board[0][coordinates["b"]] = template.format(whiteFigures["WKn"])
...
def can_do_this(figure, index, letter, d_index, d_letter):
    if figure == blackFigures["BKn"] or figure == whiteFigures["WKn"]:
        try:
            if (index + 2) == d_index and str(chr(ord(letter) + 1)) == d_letter or\
                    (index + 2) == d_index and str(chr(ord(letter) - 1)) == d_letter or\
                    (index + 1) == d_index and str(chr(ord(letter) - 2)) == d_letter or\
                    (index - 1) == d_index and str(chr(ord(letter) - 2)) == d_letter or\
                    (index - 2) == d_index and str(chr(ord(letter) - 1)) == d_letter or\
                    (index + 2) == d_index and str(chr(ord(letter) + 1)) == d_letter or\
                    (index + 1) == d_index and str(chr(ord(letter) + 2)) == d_letter or \
                    (index + 1) == d_index and str(chr(ord(letter) + 2)) == d_letter:
                return True
        except Exception:
            return False
    #return True
...
def get_correct_step():
    while True:
        ...
        if not can_do_this(selected_figure, selected_index, selected_letter, destination_index, destination_letter):
            print("You cant move that figure to " + destination_letter + " " + str(destination_index + 1) )
            continue
